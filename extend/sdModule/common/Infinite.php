...
class Infinite
{
     ...
     * @param int|array $appointField 指定某个id的子集或某个字段值的子集(数组)，如：['title' => '视频']
     * @param bool      $isTwo        是否把结果转换成二维数组
     * @return array
     */
    public function handle($data, $appointField = 0, $isTwo = false): array
    {
        $newData = $Infinite = [];

//        把所有的键名改为主键
        foreach ($data as $v) {
            $newData[$v[$this->primaryKey]] = $v;
        }

//       重组成无限极
        foreach ($newData as $key => $value) {
            if (!is_null($this->call)) $newData[$key] = ($this->call)($value);

            $this->recordInherit($newData, $key);
            $Infinite = array_merge($Infinite, $this->treeValue($newData, $key, $appointField));
        }
...
//            再次重组成二维数组
            ...
            return $result ?: [];
        } else if ($this->afterCall) {
            return $this->afterHandleCall($Infinite);
        }
     ...
     * @return mixed
     */
    public function reveres($data)
    {
        $this->recombination($data, $newData);
